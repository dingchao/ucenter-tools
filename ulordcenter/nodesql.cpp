...
int   ParseQuest(const TcpConnectionPtr & tcpcli,const std::string &buf, LengthHeaderCodec& codec )
{
    ...
    if(mstquest._questtype == MST_QUEST_ONE)
    {
       ...
       if(FindFromMasterNodeMemory( mstquest.GetKey(), vecnode  )==true)
       {
           // find        
       ...
       else 
       {
           ...
           ReadMasterNodeToNet(pConn, mstquest, vecnode); 
           if(vecnode.size()==0)
           {
              AddMasterNodeMemory(mstquest.GetKey() , false  );
              CMstNodeData   node(0,mstquest._txid,mstnodequest._voutid);
              node._validflag=0;
           ...
           {
               AddMasterNodeMemory(mstquest.GetKey() , vecnode[0]  );
           } 
    ...
    {
    	cout << "write in msg Masternode " << node.GetKey() << endl << "valid flag = " << node._validflag << endl;
...
void ReadAllNodeToNet(sql::Connection * con,std::vector<CMstNodeData>& vecnode )
{
    ...
    while (resultSet->next()) {
        ...
        mstnode._pubkey    = resultSet->getString(2);
        mstnode._txid      = resultSet->getString(4);
		mstnode._voutid	   = resultSet->getInt(5);
        mstnode._hostname  = resultSet->getString(6);
        mstnode._hostip   = resultSet->getString(7);
		mstnode._validflag = resultSet->getInt(13);
		mstnode._licperiod = resultSet->getInt(14);
		mstnode._licence = resultSet->getString(15);
		mstnode._pubkey    = resultSet->getString(16);
        vecnode.push_back(mstnode);
        AddMasterNodeMemory(mstnode.GetKey(), mstnode._validflag);
        cout<<<<" txid " << mstnode._txid << " outid  "<< mstnode._voutid <<" validate "<<mstnode._licperiod<<endl;
        i++;
		cout<<"certificate:" <<SignMessage(mstnode._txid, mstnode._voutid, mstnode._pubkey, mstnode._licperiod) <<endl;
    }
...
void  ReadMasterNodeToNet(sql::Connection * con, mstnodequest& node,std::vector<CMstNodeData>& vecnode)
{
    char sqlBuf[500] = {""};

    sprintf(sqlBuf ,"select *from ulord_master where txid =\'%s\' and outid =\'%u\'",
 ...
 */
    ...
    while (resultSet->next()) {
		...
		mstnode._licence = resultSet->getString(15);
		mstnode._pubkey    = resultSet->getString(16);
        cout<<" txid " << mstnode._txid << " outid  "<< mstnode._outid <<" validate "<<mstnode._validtime<<endl;
        vecnode.push_back(mstnode);
        i++;
    }
...
>>>>>>> ad66279a6a8a30094f701eaf4f58dab98abae2bb
